...
import { CoastResult, CoastInputs, FiResult, LongevityResult, BaristaResult, fmtUSDShort } from "./coastfire";
...
export function drawBaristaChart(canvas: HTMLCanvasElement, result: BaristaResult, currentAge: number) {
  const dpr = window.devicePixelRatio || 1;
  const cssWidth = canvas.clientWidth || 600;
  const cssHeight = 260;
  canvas.width = cssWidth * dpr;
  ...
  ctx.strokeStyle = result.meetsTarget ? "#e8a33d" : "#c96a4a";
  ctx.lineWidth = 2.5;
  ctx.lineJoin = "round";
  ctx.beginPath();
  series.forEach((p, i) => {
  ...
  if (result.depletionAge !== null) {
    const cx = xForAge(result.depletionAge);
    ctx.save();
    ctx.strokeStyle = "rgba(201,106,74,0.5)";
    ctx.setLineDash([3, 3]);
    ctx.beginPath();
    ctx.moveTo(cx, padT);
    ctx.lineTo(cx, padT + h);
